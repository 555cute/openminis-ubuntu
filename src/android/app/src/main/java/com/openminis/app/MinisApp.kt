...
class MinisApp : Application(), ImageLoaderFactory {
    ...
    override fun onCreate() {
        ...
        // [OpenMinis Code] Defensive boot: the Application class is
        // constructed before any crash dialog can render, so an
        // uncaught throw here produces exactly the "white screen
        // flash crash" the user reported. Wrap the body that follows
        // so a single bad singleton (Room migration, native lib load,
        // missing R class on a stripped manifest, etc.) leaves the
        // app in a degraded but reachable state instead of
        // silently killing the process. Logcat still gets the stack
        // via ACRA + the chained UncaughtExceptionHandler below.
        try {
            safeOnCreate()
        } catch (t: Throwable) {
            Log.e("MinisApp", "Application init failed; running with degraded state: ${t.message}", t)
            // Write a minimal breadcrumb so CrashFrequencyDetector can
            // surface it next launch — and so logcat shows the real
            // root cause when the user grabs it.
            try {
                val logs = java.io.File(filesDir, "logs")
                logs.mkdirs()
                java.io.File(logs, "boot-failure-${System.currentTimeMillis()}.log").writeText(
                    buildString {
                        appendLine("OpenMinis Code boot failure")
                        appendLine("applicationId=${com.openminis.app.BuildConfig.APPLICATION_ID}")
                        appendLine("versionName=${com.openminis.app.BuildConfig.VERSION_NAME}")
                        appendLine("versionCode=${com.openminis.app.BuildConfig.VERSION_CODE}")
                        appendLine("androidVersion=${android.os.Build.VERSION.RELEASE} (SDK ${android.os.Build.VERSION.SDK_INT})")
                        appendLine("device=${android.os.Build.MANUFACTURER} ${android.os.Build.MODEL}")
                        appendLine()
                        appendLine("Stack trace:")
                        t.printStackTrace().let { appendLine(it) }
                    }
                )
            } catch (_: Throwable) {
                // best-effort
