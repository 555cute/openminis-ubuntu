...
android {
    ...
    defaultConfig {
        // [OpenMinis Ubuntu Agent] side-by-side with stock Minis
        // (com.openminis.app) and the earlier Code fork
        // (com.openminis.code). Kotlin namespace stays
        // com.openminis.app.* — only the install id changes.
        applicationId = "com.openminis.ubuntu"
        minSdk = 26
        targetSdk = 35
        versionCode = 31
        versionName = "0.1.1-ubuntu"
    ...
    androidResources {
        noCompress += listOf("tar.gz", "proot-aarch64", "ubuntu-base-rootfs.tar.gz")
    }
...
//
// [OpenMinis Code] The `.claude/skills/debug-server/` tree is a PRIVATE
// artifact of the upstream Minis dev tree; the public mirror doesn't
// ship it. Skip the entire task (and the asset-merge dependency) when
// the directory is absent so forks without the skill can still build.
val stageDebugSkillAssets by tasks.registering(Exec::class) {
    ...
    inputs.dir(skillDir).skipWhenEmpty()
    inputs.file(script).optional()
    outputs.dir(layout.projectDirectory.dir("src/debug/assets/debug-skill")).optional()
